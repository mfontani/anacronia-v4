...
our $IACSBCOMPRESS2IACSE = sprintf( "%c%c%c%c%c", 255, 250, 86, 255, 240 );
        ...
        if ( $iac == 2 ) {                       # Got IAC DO/DONT, waiting on option
            if ( defined $TELOPTIONS{ ord $char } ) {
                ...
                if ( $do == 1 ) {
                    print " => IAC DO!!";
                    if ( ord $char == 86 ) {
                        compress2start( $kernel, $sock, $heap );
                        print " OK COMPRESS2 STARTS";
                    }
                } elsif ( $do == 0 ) {
                    print " => IAC DONT!!";
                    if ( ord $char == 86 ) {
                        compress2end( $kernel, $sock, $heap );
                        print " OK COMPRESS2 STOPS";
                    }
                }
                $do = -1;
            } else {
...
sub compress2start {
    my ( $kernel, $client, $heap ) = @_;
    $outbuffer{$client} .= $IACSBCOMPRESS2IACSE;
    warn "compress2start, creating zstream..";
    my ( $d, $initstatus ) = deflateInit( -Level => Z_BEST_COMPRESSION );
    if ( !defined $d ) {
        warn "Client $client: undefined deflateInit: status ", $initstatus, "\n";
...
sub compress2end {
    my ( $kernel, $client, $heap ) = @_;
    return if ( !defined $heap->{zstream} );
    kprint( $kernel, $client, $heap, "compress ends\r\n" );
    deflateEnd( $heap->{zstream} );
    $heap->{mccp} = 0;
}

sub kprint {
    my ( $kernel, $client, $heap, $data ) = @_;
    if ( !defined $heap->{mccp} || !$heap->{mccp} ) {
        warn "Sending plain text: $data";
    ...
    # MCCP on
    warn "Sending via MCCP: ", length $data, "\n";
    if ( !defined $heap->{zstream} ) {
        warn "No zstream associated with $client!!";
        $kernel->yield( event_error => $client );
        return;
    }
    my ( $outdeflate, $deflatestatus ) = $heap->{zstream}->deflate($data);
    if ( !defined $outdeflate ) {
        warn "Client $client: undefined deflate: status ", $deflatestatus, "\n";
        $kernel->yield( event_error => $client );
        return;
    }
    warn "Via mccp, deflated is ", length $outdeflate, "; added to buffer!";
    $outbuffer{$client} .= $outdeflate;
    my ( $outflush, $flushstatus ) = $heap->{zstream}->flush(Z_SYNC_FLUSH);
    if ( !defined $outflush ) {
        warn "Client $client: undefined flush: status ", $flushstatus, "\n";
        $kernel->yield( event_error => $client );
        return;
    }
    warn "Via mccp, flushed is ", length $outflush, "; added to buffer!";
    $outbuffer{$client} .= $outflush;
...
sub client_read {
    ...
    $data = analyze_data( $data, $client, $_[HEAP], $kernel );
    $inbuffer{$client} .= $data;
    while ( $inbuffer{$client} =~ s/(.*\n)// ) {
        warn "Got: $1, sending back";
        kprint( $kernel, $client, $_[HEAP], $1 );
...
sub client_write {
    ...
    kprint( $kernel, $client, $_[HEAP], $LOREM->paragraphs( int rand(5) + 3 ) );
    ...
    warn "Sent less bytes than thought: $datalength vs $rv";
    $kernel->yield( event_error => $client );
}
