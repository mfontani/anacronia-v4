...
my $nondata = Av4::POE::Filter::NonData->new(
    ...
    DEFAULT => sub {
        my ( $self, $telopt, $option ) = @_;
        push @nondata_parsed,
          My::Av4::Test->new( handled => 0, telopt => $telopt, option => $option );
    },
);
...
my $nto = Net::Telnet::Options->new(
    NAWS => {
        WILL => sub {
            ...
            #my ( $self, $telopt, $option ) = @_;
            push @telopt_parsed,
              My::Av4::Test->new( handled => 1, telopt => 'will', option => 'naws' );
        },
    },
);
...
if (@nondatanew_parsed) {
    warn("First member of nondatanew_parsed is a " . ref $nondatanew_parsed[0]);
}
